...
case class ShowCommentsTableMessagePlanner(override val planContext: PlanContext) extends Planner[String] {
  override def plan(using planContext: PlanContext): IO[String] = {
    val query = s"SELECT comment_id, sender_name, goods_id, time, content FROM goods.comments"
