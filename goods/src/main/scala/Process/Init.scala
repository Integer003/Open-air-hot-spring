...
object Init {
  def init(config: ServerConfig): IO[Unit] = {
    ...
    for {
      ...
      _ <- writeDB(s"CREATE TABLE IF NOT EXISTS ${schemaName}.comments (comment_id SERIAL PRIMARY KEY, sender_name VARCHAR(255) NOT NULL, goods_id INT NOT NULL, time VARCHAR(255) NOT NULL, content TEXT NOT NULL)", List())
    } yield ()
  }
}
