...
object Init {
  def init(config: ServerConfig): IO[Unit] = {
    ...
    for {
      ...
      _ <- writeDB(s"CREATE TABLE IF NOT EXISTS ${schemaName}.goods_info (goods_id SERIAL PRIMARY KEY, goods_name VARCHAR(255) NOT NULL, price DECIMAL NOT NULL, description VARCHAR(255), condition VARCHAR(50), seller_name VARCHAR(255) NOT NULL, buyer_name VARCHAR(255), verify VARCHAR(255), comment TEXT, star DECIMAL NOT NULL, image_url TEXT)", List())
